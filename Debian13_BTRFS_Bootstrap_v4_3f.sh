...
preflight_summary() {
    ...
    echo "  Live UTC:        $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee -a "$LOG_FILE"
...
sync_system_clock() {
    INFO "Checking live environment system clock..."

    local now_epoch=""
    now_epoch="$(date -u +%s 2>/dev/null || true)"
    echo "  Current UTC:     $(date -u '+%Y-%m-%d %H:%M:%S UTC' 2>/dev/null || echo unknown)" | tee -a "$LOG_FILE"

    if command -v timedatectl >/dev/null 2>&1; then
        INFO "Trying systemd-timesyncd clock synchronization..."
        ...
        for _i in {1..12}; do
            if timedatectl show -p NTPSynchronized --value 2>/dev/null | grep -qx "yes"; then
                SUCCESS "System clock synchronized with NTP"
                echo "  Synced UTC:      $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee -a "$LOG_FILE"
                return 0
            fi
            sleep 2
        done
        WARN "NTP did not report synchronized status yet"
    fi

    if command -v curl >/dev/null 2>&1; then
        INFO "Trying HTTPS Date header fallback from Debian mirror..."
        local remote_date=""
        remote_date="$(curl -fsI https://deb.debian.org/debian/ 2>/dev/null | awk -F': ' 'tolower($1)=="date"{print $2}' | tr -d '\r' || true)"
        if [[ -n "$remote_date" ]]; then
            INFO "Mirror UTC header: ${remote_date}"
            if date -u -s "$remote_date" >>"$LOG_FILE" 2>&1; then
                SUCCESS "System clock updated from HTTPS mirror header"
                echo "  Updated UTC:     $(date -u '+%Y-%m-%d %H:%M:%S UTC')" | tee -a "$LOG_FILE"
                return 0
            fi
        fi
    fi

    WARN "Could not positively synchronize the live clock automatically"
    WARN "If apt fails with 'Not live until', verify the server clock/RTC in firmware or the live environment."
}
...
bootstrap_base_system() {
    run_visible "Bootstrapping Debian ${DEBIAN_RELEASE} into ${TARGET}" \
        debootstrap --arch amd64 --include=debian-archive-keyring,ca-certificates "$DEBIAN_RELEASE" "$TARGET" https://deb.debian.org/debian
}
...
ensure_chroot_resolver() {
    INFO "Ensuring DNS resolution inside chroot..."
    [[ "$DRY_RUN" == true ]] && return 0
    ...
    if [[ -f /etc/resolv.conf ]]; then
        ...
        SUCCESS "Copied live environment resolver configuration into ${TARGET}/etc/resolv.conf"
    else
        WARN "Live environment /etc/resolv.conf was not found; chroot DNS may fail"
    fi
}


refresh_target_archive_keyring() {
    INFO "Refreshing Debian archive keyring inside ${TARGET}..."
    [[ "$DRY_RUN" == true ]] && return 0

    mkdir -p "$TARGET/usr/share/keyrings" "$TARGET/etc/apt/trusted.gpg.d"

    if [[ -f /usr/share/keyrings/debian-archive-keyring.gpg ]]; then
        cp -f /usr/share/keyrings/debian-archive-keyring.gpg "$TARGET/usr/share/keyrings/debian-archive-keyring.gpg"
        SUCCESS "Copied debian-archive-keyring.gpg into target"
    else
        WARN "Live environment keyring /usr/share/keyrings/debian-archive-keyring.gpg not found"
    fi

    if compgen -G "/etc/apt/trusted.gpg.d/debian-archive-*.asc" >/dev/null; then
        cp -f /etc/apt/trusted.gpg.d/debian-archive-*.asc "$TARGET/etc/apt/trusted.gpg.d/" 2>/dev/null || true
        SUCCESS "Copied Debian trusted key fragments into target"
    else
        WARN "No Debian trusted key fragments found under /etc/apt/trusted.gpg.d"
    fi
}
...
chroot_error() {
    local line="\$1"
    local cmd="${2:-unknown}"
    echo "[ERROR] Non-interactive chroot failed at line \${line}: \${cmd}"
    exit 1
}
trap 'chroot_error \$LINENO "\$BASH_COMMAND"' ERR

echo "[INFO] Starting non-interactive chroot phase"
echo "[INFO] Hostname=\$HOSTNAME User=\$USERNAME Timezone=\$TIMEZONE"
...
echo "[INFO] Running apt-get update inside chroot"
apt-get update

echo "[INFO] Installing base packages inside chroot"
apt-get install -y \
    linux-image-amd64 \
    grub-efi-amd64 \
    efibootmgr \
...
if [[ "\$STATIC_IP_ENABLED" == "true" ]]; then
    echo "[INFO] Installing static networking packages"
...
append_line_if_missing "\$USER_HOME/.bashrc" "$CUSTOM_PS1"
...
echo "[INFO] Non-interactive chroot phase completed"
...
run_chroot_config() {
    ...
    INFO "Running non-interactive chroot setup in ${TARGET}"
    if ! chroot "$TARGET" /usr/bin/env bash -x /root/bootstrap-chroot-noninteractive.sh 2>&1 | tee -a "$LOG_FILE"; then
        ERROR "Non-interactive chroot exited before completion. Last 80 lines of installed log:"
        if [[ -f "$TARGET/var/log/debian13-bootstrap.log" ]]; then
            tail -80 "$TARGET/var/log/debian13-bootstrap.log" | tee -a "$LOG_FILE" >&2 || true
        fi
        die "Non-interactive chroot failed. Review $TARGET/var/log/debian13-bootstrap.log"
    fi

    [[ -f "$TARGET/root/bootstrap-chroot-noninteractive.done" ]] || {
        ERROR "Non-interactive chroot exited without writing completion marker."
        if [[ -f "$TARGET/var/log/debian13-bootstrap.log" ]]; then
            tail -80 "$TARGET/var/log/debian13-bootstrap.log" | tee -a "$LOG_FILE" >&2 || true
        fi
        die "Non-interactive chroot did not complete successfully."
    }
...
post_install_validation() {
    ...
    local user_bashrc
    if [[ "$USERNAME" == "root" ]]; then
        user_bashrc="$TARGET/root/.bashrc"
    else
        user_bashrc="$TARGET/home/$USERNAME/.bashrc"
    fi
    test -f "$user_bashrc" && grep -Fq "export PS1=" "$user_bashrc" && SUCCESS "User .bashrc prompt configured" || WARN "User .bashrc prompt missing"
